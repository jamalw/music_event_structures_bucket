...
fmri_filename1 = datadir + 'subjects/' + subjs + '/analysis/run1.feat/trans_filtered_func_data.nii'
fmri_filename2 = datadir + 'subjects/' + subjs + '/analysis/run2.feat/trans_filtered_func_data.nii'
mask_filename = datadir + 'data/a1plus_2mm.nii' 
bg_img = datadir + 'data/MNI152_T1_2mm_brain.nii'

masker = NiftiMasker(mask_img=mask_filename, standardize=True)

# We give the masker a filename and retrieve a 2D array ready
# for machine learning with scikit-learn
fmri_masked1 = masker.fit_transform(fmri_filename1)
fmri_masked2 = masker.fit_transform(fmri_filename2)


# Create labels
labels = pandas.read_csv(datadir + 'subjects/' + subjs + '/run1model.csv',header = 0)
target = []
