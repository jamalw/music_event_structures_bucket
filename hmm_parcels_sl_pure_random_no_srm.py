...
for i in range(int(np.max(parcels))):
    ...
    print("Fitting HMM")
    SL_match,true_bounds = HMM(data,human_bounds)

    # save true hmm fit for each ROI
    np.save(savedir + '/parcel_hmm_bounds/parcel' + str(i+1),true_bounds)   
