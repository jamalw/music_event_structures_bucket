...
def searchlight(coords,human_bounds,mask,subjs,song_idx,song_bounds,srm_k,hrf):
    ...
    mfcc1   = np.load(datadir + 'mfccRun1_hrf.npy')[0:12,:]
    mfcc2   = np.load(datadir + 'mfccRun2_hrf.npy')[0:12,:]
    tempo1  = np.load(datadir + 'tempoRun1_hrf_12PC_singles.npy')
    tempo2  = np.load(datadir + 'tempoRun2_hrf_12PC_singles.npy')
        ...
        for y in range(0,np.max(coords, axis=0)[1]+stride,stride):
           for z in range(0,np.max(coords, axis=0)[2]+stride,stride):
               ...
               for i in range(len(subjs)):
                   ...
                   motion = subj_regs.T
                   fullRegs1 = np.vstack((motion[:,0:2511],chroma1,mfcc1,tempo1))
                   regr = linear_model.LinearRegression()
                   regr.fit(fullRegs1.T,subj_data[:,:,0].T)
                   subj_data1 = subj_data[:,:,0] - np.dot(regr.coef_, fullRegs1) - regr.intercept_[:, np.newaxis] 
                   data.append(np.nan_to_num(stats.zscore(subj_data1,axis=1,ddof=1)))
               for i in range(len(subjs)):
                   subj_data = np.load(datadir + subjs[i] + '/' + str(x) + '_' + str(y) + '_' + str(z) + '.npy')
                   subj_regs = np.genfromtxt(datadir + subjs[i] + '/EPI_mcf2.par')
                   motion = subj_regs.T
                   fullRegs2 = np.vstack((motion[:,0:2511],chroma2,mfcc2,tempo2))
                   regr = linear_model.LinearRegression()
                   regr.fit(fullRegs2.T,subj_data[:,:,1].T)
                   subj_data2 = subj_data[:,:,1] - np.dot(regr.coef_, fullRegs2) - regr.intercept_[:, np.newaxis]
...
if runNum == 0:
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/raw/globals_raw_test_run1_split_merge_regress_all_tempo_12_PC_singles_GSBS_no_srm', results3d_real)
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/zscores/globals_z_test_run1_split_merge_regress_all_tempo_12_PC_singles_GSBS_no_srm', results3d)
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/perms/globals_z_test_run1_split_merge_regress_all_tempo_12_PC_singles_GSBS_no_srm', results3d_perms)
if runNum == 1:
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/raw/globals_raw_test_run2_split_merge_regress_all_tempo_12_PC_singles_GSBS_no_srm', results3d_real)
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/zscores/globals_z_test_run2_split_merge_regress_all_tempo_12_PC_singles_GSBS_no_srm', results3d)
    np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_human_bounds_fit_to_all/' + songs[song_idx] +'/perms/globals_z_test_run2_split_merge_regress_all_tempo_12_PC_singles_no_srm', results3d_perms)
