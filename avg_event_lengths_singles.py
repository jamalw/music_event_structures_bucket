...
for i in range(len(durs)):
    ...
    fn_z = datadir + songs[i] + '/avg_data/globals_avg_z_n25_k' + str(K) + '.npy'
    print(fn)
    data = np.load(fn)
    data_z = np.load(fn_z)
    events[:,:,:] += data/len(durs)
    events_z[:,:,:] += data_z/len(durs) 
...
# Plot and save searchlight results
maxval = np.max(events_z)
minval = np.min(events_z)
img = nib.Nifti1Image(events_z, affine=nii_template.affine)
img.header['cal_min'] = minval
img.header['cal_max'] = maxval
nib.save(img,datadir + str(event_length) + '_sec_events_Z.nii.gz') 
