...
def HMM(X,human_bounds,song_idx,song_bounds,srm_k,hrf):
    ...
    for p in range(nPerm+1):
        # scoring function: sum of minimum distances
        match[p] = sum([np.min(np.abs(perm_bounds - hb)) for hb in human_bounds])
        # scoring function: euclidean distance 
        #match[p] = np.sqrt(sum([np.min((perm_bounds - hb)**2) for hb in human_bounds]))
        # scoring function: any human bound within w TRs of permuted bound 
...
np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_bound_match_shuffle_event_lengths_rotation/' + songs[song_idx] +'/raw/globals_raw_srm_k_' + str(srm_k) + '_train_run1_euclidean1', results3d_real)
np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_bound_match_shuffle_event_lengths_rotation/' + songs[song_idx] +'/zscores/globals_z_srm_k' + str(srm_k) + '_train_run1_euclidean1', results3d)
np.save('/jukebox/norman/jamalw/MES/prototype/link/scripts/data/searchlight_output/HMM_searchlight_bound_match_shuffle_event_lengths_rotation/' + songs[song_idx] +'/perms/globals_z_srm_k' + str(srm_k) + '_train_run1_euclidean1', results3d_perms)
